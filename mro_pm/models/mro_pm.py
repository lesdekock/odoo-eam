...
class MroPmParameter(models.Model):
...
class MroPmMeter(models.Model):
    ...
    def _get_utilization(self):
        for meter in self:
            ...
            meter_line_ids = meter_line_obj.search(
                [('meter_id', '=', meter.id), ('date', '<=', time.strftime('%Y-%m-%d', time.gmtime(Da)))], limit=1,
                order='date desc')
            ...
            number = 0
            Us = 0
            meter_line_ids = meter_line_obj.search([('meter_id', '=', meter.id), ('date', '>', meter_line.date)],
                                                   order='date')
    ...
    date = fields.Date(related='meter_line_ids.date', string='Date')
    ...
    def get_reading(self, date):
        ...
        meter = self
        meter_line_obj = self.env['mro.pm.meter.line']
        prev_read = meter_line_obj.search([('meter_id', '=', meter.id), ('date', '<=', date)], limit=1,
                                          order='date desc')
    ...
    def create(self, vals):
        ...
        meter_id = super(MroPmMeter, self).create(vals)
        values = {
            ...
            if vals.get('new_value', False) and meter.state == 'reading':
                if meter.reading_type == 'inc':
                    ...
                    if meter.parent_meter_id.reading_type == 'inc':
                        ...
                        vals.update({'new_value': 0})
        return super(MroPmMeter, self).write(vals)
    ...
    def activate_meter(self):
        for meter in self:
            ...
            if meter.reading_type == 'cng':
                ...
                vals['new_value'] = meter.value
            if meter.reading_type != 'src': vals['parent_meter_id'] = False
            meter.write(vals)
        return True
...
class MroPmMeterLine(models.Model):
...
class MroPmMeterRatio(models.Model):
...
class MroPmMeterInterval(models.Model):
    ...
    def _get_name(self):
        for interval in self:
            if interval.interval_min == interval.interval_max:
                interval.name = str(interval.interval_min)
            else:
                interval.name = str(interval.interval_min) + ' - ' + str(interval.interval_max)
...
class MroPmRule(models.Model):
    _name = "mro.pm.rule"
    _description = "Preventive Maintenance Rule"

    name = fields.Char('Name', size=64)
    active = fields.Boolean('Active',
                            help="If the active field is set to False, it will allow you to hide the PM without removing it.",
                            default=True)
    ...
    def create(self, vals):
        ...
        return super(MroPmRule, self).create(vals)


class MroPmRuleLine(models.Model):
